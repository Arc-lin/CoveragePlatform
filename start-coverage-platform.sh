...
# CoveragePlatform 启动脚本
# 用法：./start-coverage-platform.sh [YOUR_LOCAL_IP]
# 例如：./start-coverage-platform.sh 192.168.0.113
...
start_frontend() {
    ...
    export HOST=0.0.0.0
    export BROWSER=none
    export REACT_APP_API_URL="http://$LOCAL_IP:3001/api"
    
    # 后台启动前端
    npm start > /tmp/coverage-frontend.log 2>&1 &
    FRONTEND_PID=$!
